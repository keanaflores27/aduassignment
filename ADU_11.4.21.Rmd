...
```{r}
centered_model <- svyglm(RENT ~ YEAR + FTOTINC + UNITSSTR + Black + Native + Asian + Other + Multiple, design = centered_data)
summary(centered_model)
```

```{r}
data_scaled <- data %>%
  mutate(FTOTINC = FTOTINC/1000)
```

```{r}
scaled_model <- svyglm(RENT ~ YEAR + FTOTINC + UNITSSTR + Black + Native + Asian + Other + Multiple, design = data_scaled)
...
```{r}
log_model <- svyglm(RENT ~ YEAR + FTOTINC + UNITSSTR + Black + Native + Asian + Other + Multiple, design = data_log)
summary(log_model)
```